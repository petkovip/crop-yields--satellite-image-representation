...
For the full write-up (motivation, related work, methods, results, limitations, references) see `[final-report/DL_geo-simclr_REPORT.pdf](final-report/DL_geo-simclr_REPORT.pdf)`. For a brief presentation, see `[final-report/DL_geo-simclr_PRESENTATION.pdf](final-report/DL_geo-simclr_PRESENTATION.pdf)`.
...
│
├── final-report/              ← all submission artefacts (PDFs + assets)
│   ├── DL_geo-simclr_REPORT.pdf                   ← final report (PDF)
│   ├── DL_geo-simclr_PRESENTATION.pdf             ← brief video-presentation slides (PDF)
│   ├── references.bib                             ← BibTeX bibliography (cited by the report)
...
│   └── 09_results.ipynb
│
├── readings/                  ← reference papers + my notes on them
│   ├── Satellite Data Notes.md     ← initial scoping notes built from the readings below
│   ├── CropNet Dataset paper.pdf   ← Lin et al. KDD 2024 (the dataset we use)
│   ├── SimCLR paper.pdf            ← Chen et al. ICML 2020 (the contrastive recipe)
│   ├── GeoSpatial-Big-Data.pdf     ← background reading on EO + remote sensing
│   └── CNN-RNN Framework for Crop Yield Prediction/   ← supporting code/data from prior work
│
...
1. **Direct copy.** Available upon request.
2. **Reproduce from scratch.** The notebooks regenerate the full pipeline deterministically from public sources (CropNet HuggingFace + USDA NASS open data). Run them in order: `02 → 03 → 04 → 05 → 06 → 09`. The first part of `02_acquire_data.ipynb` covers only three states - IA, IL, IN (78 GB) for users with limited storage; the second part expands to seven states by adding MN, NE, OH, and SD (260 GB total).
...
- For results, the headline numbers, the narrative, and the limitations discussion → see `[final-report/DL_geo-simclr_REPORT.pdf](final-report/DL_geo-simclr_REPORT.pdf)`.
- For a brief video presentation → `[final-report/DL_geo-simclr_PRESENTATION.pdf](final-report/DL_geo-simclr_PRESENTATION.pdf)`.
